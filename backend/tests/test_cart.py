...
def test_redis_down_returns_503(api, make_product, monkeypatch):
    product = make_product(stock=10)
...
def test_merge_sums_quantities_and_render_caps_to_stock(make_product, settings):
    settings.CART_REDIS_URL = "redis://redis:6379/15"
    product = make_product(stock=5)
